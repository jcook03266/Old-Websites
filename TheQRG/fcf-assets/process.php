...
try {
    ...
    if (strtoupper(USE_SMTP) == "YES") {
        ...
        if (strtoupper(SMTP_AUTH) == "YES") {
            ...
            if (strtoupper(SMTP_SECURE) == "STARTTLS") {
...
{
    return getEmailBody('htm');
}

function getPlainBody()
{
    return getEmailBody('txt');
}

function getEmailBody($type)
{
    if ($type == "htm") {
        $ss = "b";
        $body = file_get_contents('./templates/' . EMAIL_TEMPLATE_IN_HTML);
    } else {
        $ss = "a";
        $body = file_get_contents('./templates/' . EMAIL_TEMPLATE_IN_TEXT);
    }
    foreach ($_POST as $field => $value) {
        if ($field == "recaptcha-token") {
            continue;
        }
        $field_to_find = "{" . $field . "}";
        if ($type == "htm") {
    ...
    $body = str_replace('{IP}', getUserIp(), $body);
...
function validateFields($form_fields, $mapping)
{
    ...
    foreach ($form_fields as $field => $type) {
        ...
        $validate->validate($field, $display_name, $_POST[$field], $type);
    }
